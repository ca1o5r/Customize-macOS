...
rm -r /opt/ExtenalDisk/System/Applications/Music.app
rm -r /opt/ExtenalDisk/System/Applications/Weather.app

cd $(find /private/var/folders -name com.apple.dock.launchpad)/db

sqlite3 db "Delete From items Where rowid In (Select item_id From apps Where title In ('Chess','News','QuickTime Player','Stocks','TV','Home','Calculator','Notes','Voice Memos','Contacts','Freeform','Reminders','Maps','FaceTime','Stickies','Photo Booth','Messages','Find My','Dictionary','Clock','Calendar','Feedback Assistant','Automator','Shortcuts','Siri','Time Machine','AirPort Utility','Audio MIDI Setup','ColorSync Utility','Digital Color Meter','Grapher','VoiceOver Utility','Migration Assistant','Boot Camp Assistant','Image Capture','Mission Control','Bluetooth File Exchange','Mail','Music','Weather','股市','国际象棋','家庭','计算器','便笺','备忘录','词典','查找','地图','FaceTime通话','日历','时钟','提醒事项','通讯录','无边记','信息','语音备忘录','AirPort实用工具','快捷指令','旁白实用工具','启动转换助理','迁移助理','色彩同步实用工具','时间机器','数码测色计','音频MIDI设置','自动操作','图像捕捉','调度中心','蓝牙文件交换','邮件','音乐','天气'))"
